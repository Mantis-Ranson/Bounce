...
export default class SinglePost extends React.Component{
	...
	render(){
		const self = this;
		const me = self.props.LoginState.loggedIn.username;
		if(this.state.SinglePost.post){
			if(this.state.SinglePost.post.error){
				return (
					<div id="singlepost">
			        <h1>{this.state.SinglePost.post.error}</h1>
			        </div>	
					);
			}
			let PostArray = []; 
			PostArray.push(self.state.SinglePost.post)
			return(	
			<div id="singlepost">
